"""
main2.py - FPV_GateHill (source2) RECOVERY-/OTA-ONLY-Firmware

Notfall-Skript fuer sehr alte oder kaputte Installationen der schlanken
Tor/Huegel-Firmware (source2): Startet NUR den WLAN-Hotspot und eine
minimale OTA-Update-Seite - keine Telemetrie, kein Score-Tracking. Die
komplette OTA-Seite ist als Python-String in DIESEM Skript eingebettet
(kein index.html noetig), damit das Update auch funktioniert, wenn auf
dem Pico gar keine oder nur sehr alte/kaputte Dateien liegen.
...
UMBENENNEN (oder gleich als "main.py" hochladen):
  ...
  6. Lade entweder eine einzelne main.py/koth_mode.py/race_mode.py Datei
     hoch, oder - empfohlen - ein komplettes Firmware-Bundle
     "gatehill.nbo" (siehe build_firmware.py). Das Bundle wird
     automatisch entpackt und ersetzt main.py + index.html + koth_mode.py
     + race_mode.py auf einmal.
...
# keine beliebigen Dateinamen vom Client) - eigene, schlanke Whitelist fuer
# die source2-Firmware (nur Tor/Huegel-Logik, keine Trick-/Infection-Dateien).
OTA_ALLOWED_TARGETS = (
...
# mehrere der obigen Dateien in einem Rutsch aktualisiert. Eigener Dateiname
# (statt "firmware.nbo"), damit ein Bundle der Haupt-Firmware nicht versehentlich
# auf einem Tor/Huegel-Geraet als Bundle erkannt wird.
...
<div class="s">
<h2>Upload Update</h2>
<div style="color:#9fb4cb;font-size:.78em;margin-bottom:6px">Allowed: main.py, index.html, koth_mode.py, race_mode.py - or a full <b>gatehill.nbo</b> bundle (recommended, see build_firmware.py)</div>
